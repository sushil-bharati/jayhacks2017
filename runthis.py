...
try:
    while flag:
        ...
        if any(words in spoken for words in temperature): #temperature update
            ...
            msg = 'I am glad you asked that. My name is Kiddo. I am built by Sushil and Manjish on Feb 17, 2017 night for Jayhacks!'
            if myAttrib == 'speak':
                engine.say(msg)
                engine.runAndWait()
            if myAttrib == 'text': client.messages.create(to=my_cell,from_=my_twilio, body=msg)
            ...
            ##
            val = listen(myAttrib)
            print ("Initiating Music..")
            ##
            strMusic = 'F:\\workout_music\\'+(val)+'.mp3'
            mixer.init()
